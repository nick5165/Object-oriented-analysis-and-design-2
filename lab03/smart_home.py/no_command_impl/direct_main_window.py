...
# Импортируем наш новый менеджер (Без паттерна Команда)
from no_command_impl.app_direct import AppDirect

# Импортируем только сами девайсы (команды больше не нужны!)
...
class DirectMainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Умный дом: Без паттерна Команда")
        self.resize(1100, 700)

        # Используем прямой контроллер вместо старого Application
        self.app = AppDirect()
    ...
    def init_ui(self, tracks):
        ...
        # === ЛЕВАЯ ЧАСТЬ (Идентична оригиналу) ===
        ...
        remote_frame.setStyleSheet("""
            QFrame { background-color: #2c3e50; border-radius: 25px; border: 3px solid #1a252f; }
        ...
        lbl_remote = QLabel("ПУЛЬТ (Без паттернов)")
        lbl_remote.setAlignment(Qt.AlignmentFlag.AlignCenter)
        lbl_remote.setFont(QFont("Arial", 14))
    ...
    # === ЛОГИКА ПУЛЬТА (БЕЗ ПАТТЕРНА КОМАНДА) ===
    ...
    def on_power_pressed(self):
        ...
        if target == "Лампа": 
            self.app.execute(self.lamp, lambda: self.lamp.turn_off() if self.lamp.is_on else self.lamp.turn_on())
        elif target == "Розетка": 
            self.app.execute(self.socket, lambda: self.socket.turn_off() if self.socket.is_on else self.socket.turn_on())
        elif target == "Пылесос": 
            self.app.execute(self.vacuum, lambda: self.vacuum.go_to_base() if self.vacuum.is_cleaning else self.vacuum.start_cleaning())
        self.sync_ui()
    ...
    def on_mode_pressed(self):
        ...
        if target == "Лампа": 
            def relax():
                ...
                self.lamp.turn_on()
            self.app.execute(self.lamp, relax)
        elif target == "Пылесос": 
            self.app.execute(self.vacuum, lambda: self.vacuum.set_work_mode("Turbo"))
        self.sync_ui()

    def on_color_pressed(self):
        if self.current_target() == "Лампа":
            self.app.execute(self.lamp, lambda: self.lamp.set_color("Red"))
        self.sync_ui()
    ...
    def on_play_pressed(self):
        if self.current_target() == "Колонка":
            ...
            if track != "Нет треков":
                def play_track():
                    self.speaker.change_track(track)
                    self.speaker.play()
                self.app.execute(self.speaker, play_track)
        self.sync_ui()

    def on_pause_pressed(self):
        if self.current_target() == "Колонка":
            self.app.execute(self.speaker, lambda: self.speaker.pause() if self.speaker.is_playing else self.speaker.play())
        self.sync_ui()

    def on_next_pressed(self):
        if self.current_target() == "Колонка":
            def next_tr():
                self.speaker.next_track()
                self.speaker.play()
            self.app.execute(self.speaker, next_tr)
        self.sync_ui()

    def on_prev_pressed(self):
        if self.current_target() == "Колонка":
            def prev_tr():
                self.speaker.prev_track()
                self.speaker.play()
            self.app.execute(self.speaker, prev_tr)
        self.sync_ui()
    ...
    def on_vol_pressed(self, is_up):
        if self.current_target() == "Колонка":
            ...
            self.app.execute(self.speaker, lambda: self.speaker.set_volume(new_vol))
        self.sync_ui()

    def on_base_pressed(self):
        if self.current_target() == "Пылесос":
            self.app.execute(self.vacuum, lambda: self.vacuum.go_to_base())
        self.sync_ui()
