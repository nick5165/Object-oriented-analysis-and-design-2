...
class MediaToggleCommand(Command):
    def execute(self) -> bool:
        if self.device.current_track == "None":
            return False
...
class SpeakerPrevCommand(Command):
    """Команда переключения на предыдущий трек."""
    def execute(self) -> bool:
        if not self.device.playlist:
            return False
        self.save_backup()
